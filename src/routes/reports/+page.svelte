<script lang="ts">
  import { products, transactions, getProductStatus, formatCurrency } from '$lib/stores';
  ...
  let importByDay = $derived(() => {
    ...
    importTxns.forEach(t => { const d = t.date.split(' ')[0]; map[d] = (map[d] || 0) + t.qty; });
    return map;
  });
  let exportByDay = $derived(() => {
    const map: Record<string, number> = {};
    exportTxns.forEach(t => { const d = t.date.split(' ')[0]; map[d] = (map[d] || 0) + t.qty; });
    return map;
  });
  let allDays = $derived(() => {
  ...
  let maxBarVal = $derived(() => {
    let m = 1;
    const im = importByDay(), ex = exportByDay();
  ...
  function exportCSV() {
    ...
    const header = 'Thời gian,Loại,Sản phẩm,Số lượng,Người thực hiện,Ghi chú\n';
    const rows = $transactions.map(t =>
      `"${t.date}","${t.type === 'import' ? 'Nhập' : 'Xuất'}","${t.product}",${t.qty},"${t.user || ''}","${t.note}"`
    ).join('\n');
...
<div class="fade-in">
  ...
  <div class="grid-stats">
    ...
    <div class="stat-card">
      ...
      <div class="stat-value">{totalImport - totalExport > 0 ? '+' : ''}{totalImport - totalExport}</div>
      <span class="stat-change" class:up={totalImport >= totalExport} class:down={totalImport < totalExport}>
        {totalImport > totalExport ? 'Nhập nhiều hơn' : totalImport < totalExport ? 'Xuất nhiều hơn' : 'Cân bằng'}
      </span>
    </div>
  </div>
  ...
  <div class="chart-card">
    ...
    <div class="chart-bars">
      {#each allDays() as day, i}
        ...
        <div class="chart-group" style="animation-delay:{i * 60}ms">
          <div class="chart-pair">
            <div class="chart-bar import" style="height:{(imVal / maxBarVal()) * 100}%" title="Nhập: {imVal}">
              {#if imVal > 0}<span class="chart-val">{imVal}</span>{/if}
            </div>
            <div class="chart-bar export" style="height:{(exVal / maxBarVal()) * 100}%" title="Xuất: {exVal}">
      ...
      {:else}
        <div style="width:100%;text-align:center;color:var(--text3);padding:40px;font-size:13px">Chưa có dữ liệu giao dịch</div>
      {/each}
    </div>
  </div>
  ...
  <div class="table-card">
    ...
    <table>
      <thead><tr><th>Mã SP</th><th>Sản phẩm</th><th>Tồn kho</th><th>Tối thiểu</th><th>Giá trị</th><th>Trạng thái</th></tr></thead>
      <tbody>
        {#each $products as p}
          ...
          <tr>
            ...
            <td class="mono" style="color:var(--accent)">{formatCurrency(p.qty * p.price)}đ</td>
    ...
    <div class="table-header">
      ...
      <div style="display:flex;align-items:center;gap:12px">
        <div class="seg-control" style="margin-right:8px">
          <button class="seg-btn" class:active={filterType === 'all'} onclick={() => filterType = 'all'}>Tất cả</button>
          <button class="seg-btn" class:active={filterType === 'import'} onclick={() => filterType = 'import'}>Nhập</button>
          <button class="seg-btn" class:active={filterType === 'export'} onclick={() => filterType = 'export'}>Xuất</button>
        </div>
        ...
        <div class="search-wrap">
          <Icon name="search" size={12} color="var(--text3)" strokeWidth={2.5}/>
          <input class="search-input" type="text" placeholder="Tìm sản phẩm..." bind:value={searchQuery} style="width:120px;font-size:12px" />
        </div>
      </div>
    </div>
    <table>
      <thead><tr><th>Thời gian</th><th>Loại</th><th>Sản phẩm</th><th>Số lượng</th><th>Người thực hiện</th><th>Ghi chú</th></tr></thead>
      <tbody>
        {#each filteredTxns as t, i}
          <tr class="row-anim" style="animation-delay:{i * 20}ms">
            <td style="color:var(--text2);font-size:12px;white-space:nowrap">{t.date}</td>
            <td>
              {#if t.type === 'import'}
        ...
        {:else}
          <tr><td colspan="6" style="text-align:center;color:var(--text3);padding:40px">Không có giao dịch nào</td></tr>
        {/each}
      </tbody>
    </table>
...
<style>
  ...
  /* ── Chart Legend ── */
  .legend { display: flex; gap: 14px; font-size: 11px; color: var(--text3); }
  ...
  .chart-bar.import { background: var(--green); }
  .chart-bar.export { background: var(--accent); }
