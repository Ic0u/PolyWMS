<script lang="ts">
  ...
  // RQ57: Force password change state
  ...
  function handleLogin() {
    ...
    // RQ57: Check if must change password
    if (foundUser.mustChangePassword) {
      changingPassword = true;
      changingUser = foundUser.username;
      return;
    }
  ...
  function handleChangePassword() {
    if (!newPass || !confirmPass) {
      showAlert("Vui lòng nhập đầy đủ mật khẩu mới.", "error");
      return;
    }
    if (newPass.length < 6) {
    ...
    const userList = get(users);
    const foundUser = userList.find((u) => u.username === changingUser);
    if (!foundUser) return;

    users.update((list) =>
      list.map((u) =>
        u.username === changingUser
  ...
  <div class="auth-right">
    <div class="form-panel">
      {#if changingPassword}
        ...
        <h2 class="auth-heading">Đổi mật khẩu</h2>
        <p
          style="color: var(--text2); font-size: 13px; text-align: center; margin-bottom: 16px;"
        >
          Mật khẩu đã được đặt lại. Vui lòng tạo mật khẩu mới.
        </p>
        ...
        <button class="btn-signin" onclick={handleChangePassword}
          >Đổi mật khẩu</button
        >
      {:else}
        <!-- Logo Container -->
        ...
        <div class="forgot-row">
          <a
            href="#"
            class="forgot-link"
            onclick={(e) => showComingSoon("Quên mật khẩu", e)}
            >Quên mật khẩu?</a
          >
        </div>
        ...
        <div class="social-row">
          <button
            class="social-btn"
            onclick={() => showComingSoon("Đăng nhập Google")}
            ><Icon name="google" size={18} /></button
          >
          <button
            class="social-btn"
            onclick={() => showComingSoon("Đăng nhập Apple")}
            ><Icon name="apple" size={18} /></button
          >
          <button
            class="social-btn"
            onclick={() => showComingSoon("Đăng nhập Facebook")}
            ><Icon name="facebook" size={18} /></button
          >
        </div>

        <p class="signup-row">
          Chưa có tài khoản? <strong
            onclick={(e) => showComingSoon("Đăng ký tài khoản", e)}
            role="button"
            tabindex="0"
            onkeydown={(e) => {
              if (e.key === "Enter") showComingSoon("Đăng ký tài khoản", e);
            }}>Đăng ký ngay</strong
          >
        </p>
        <p class="terms-row">
          Bằng cách nhấp vào nút trên, bạn đồng ý với <u>Điều khoản sử dụng</u>
          và <u>Chính sách quyền riêng tư</u>
        </p>
      {/if}
    </div>
  </div>
